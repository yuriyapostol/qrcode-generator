...
const qrcode = function(typeNumber : number = 0, errorCorrectionLevel : string = 'M') : QRCode {
  ...
  const resolveEncodingName = function(mode : EncoderMode, opts? : QRCodeAddDataOptions) {
    if (typeof opts?.encoding === 'string') {
      return opts.encoding;
    }

    const defaultEncoding = qrcode.getDefaultEncoding(mode);
    if (typeof defaultEncoding === 'string') {
      return defaultEncoding;
    }

    if (mode === 'Byte' && qrcode.getEncoder('UTF-8')) {
      return 'UTF-8';
    }

    if (mode === 'Kanji' && qrcode.getEncoder('SJIS')) {
      return 'SJIS';
    }
  ...
  const resolveEncoder = function(mode : EncoderMode, opts? : QRCodeAddDataOptions) {
    const encodingName = resolveEncodingName(mode, opts);
    if (!encodingName) {
      return undefined;
    }

    const encoder = qrcode.getEncoder(encodingName);
    if (!encoder) {
      throw 'unknown encoding: ' + encodingName;
    }

    if (encoder.modes && encoder.modes.indexOf(mode) < 0) {
      throw 'encoding not supported for mode: ' + mode + '/' + encodingName;
    }

    return encoder;
  };

  const addECI = function(opts : QRCodeAddDataOptions | undefined, encoder : QRCodeEncoder) {
    let eciAssignmentNumber;

    if (typeof opts?.eci === 'number') {
      eciAssignmentNumber = opts.eci;
    } else if (opts?.eci === true) {
      eciAssignmentNumber = encoder.eci;
      if (typeof eciAssignmentNumber !== 'number') {
        throw 'eci not supported for encoding';
      }
    }

    if (typeof eciAssignmentNumber === 'number') {
      _dataList.push(qrECI(eciAssignmentNumber) );
    }
  };
  ...
  const addData = function(data : string | QRCodeAddDataInput[], mode? : string, opts? : QRCodeAddDataOptions) {
    ...
    case 'Numeric' :
      if (typeof opts !== 'undefined') {
        throw 'options not supported for mode:' + mode;
      }
    ...
    case 'Alphanumeric' :
      ...
      break;
    case 'Byte' : {
      const encoder = resolveEncoder('Byte', opts);
      if (encoder) {
        addECI(opts, encoder);
        newData = qr8BitByte(data, encoder.encode);
      } else {
        if (typeof opts?.eci !== 'undefined') {
    ...
    case 'Kanji' : {
      const encoder = resolveEncoder('Kanji', opts);
      if (!encoder) {
        throw 'sjis not supported.';
      }
      addECI(opts, encoder);
      newData = qrKanji(data, encoder.encode);
      break;
    }
    default :
      throw 'mode:' + mode;
...
qrcode.registerEncoder = function(encoding : string, encoder : QRCodeEncoder) {
  encoders[encoding] = encoder;
};

qrcode.getEncoder = function(encoding : string) {
  return encoders[encoding];
};

qrcode.setDefaultEncoding = function(mode : EncoderMode, encoding : string) {
  if (!qrcode.getEncoder(encoding)) {
    throw 'unknown encoding: ' + encoding;
  }
  defaultEncodings[mode] = encoding;
};

qrcode.getDefaultEncoding = function(mode : EncoderMode) {
...
const qr8BitByte = function(data : string, stringToBytes? : QRCodeStringEncoder) : QRData {

  const _mode = QRMode.MODE_8BIT_BYTE;
  const _bytes = (stringToBytes || qrcode.stringToBytes)(data);
...
const qrKanji = function(data : string, stringToBytes : QRCodeStringEncoder) : QRData {
...
const qrECI = function(assignmentNumber : number) : QRData {

  const _mode = QRMode.MODE_ECI;

  if (assignmentNumber < 0 || assignmentNumber > 999999) {
    throw 'bad eci assignment number: ' + assignmentNumber;
  }
  ...
  const write = function(buffer : QRBitBuffer) {
    if (assignmentNumber < (1 << 7) ) {
      buffer.put(assignmentNumber, 8);
    } else if (assignmentNumber < (1 << 14) ) {
      buffer.put(0x8000 | assignmentNumber, 16);
    } else {
      buffer.put(0xC00000 | assignmentNumber, 24);
    }
  };
